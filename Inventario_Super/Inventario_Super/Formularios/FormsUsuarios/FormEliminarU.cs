...
{
    public partial class FormEliminarU : Form
    {
        public FormEliminarU()
        {
            InitializeComponent();
        }
        ...
        private void FormEliminarU_Load(object sender, EventArgs e)
        {
            MaximizeBox = false;
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                ConexionBD con = new ConexionBD();
                SqlCommand comando = new SqlCommand();
                comando.CommandText = "delete from Usuarios where idUsuario = @id";
                comando.Connection = con.abrirConexion();
                comando.Parameters.AddWithValue("@id",txtId.Text);
                int fila = comando.ExecuteNonQuery();
                if (fila == 1)
                {
                    MessageBox.Show("Se elimino el registro del usuario en la Base de Datos", "INFORMACIÓN", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    txtId.Clear();
                    txtId.Enabled = true;
                    btnBuscar.Enabled = true;
                }
            ...
            {
                MessageBox.Show("Ocurrio un error al eliminar el registro en la Base de Datos\n" + ex.Message, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}